...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Inside your useEffect where you fetch recipes
    const fetchRecipes = async () => {
      try {
        ...
        if (response.ok) {
          setRecipes(data);
          console.log("Recipes set in state:", data.length);
        } else {
  ...
  // Filter recipes exactly as in your original code
        ...
        >
          ...
          <motion.div className="py-8 bg-white" variants={itemVariants}>
            <div className="max-w-7xl mx-auto px-4">
              ...
              {!error && filteredRecipes.length === 0 && recipes.length > 0 && (
                <motion.div
                  className="flex flex-col items-center justify-center py-16"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
